...
if missing_strings:
    for lang_name, lang_code in LANGUAGES.items():
        ...
        print(f"Batch translating {len(missing_strings)} strings to {lang_name}...")
        try:
            ...
            translated_list = GoogleTranslator(source='en', target=google_lang).translate_batch(missing_strings)
            
            for i, s in enumerate(missing_strings):
                translated = translated_list[i] if translated_list[i] else s
                translated = translated.replace("'", "\\'").replace("$", "\\$")
                existing_data[lang_name][s] = translated
        except Exception as e:
            print(f"Batch translation failed for {lang_name}: {e}")
            for s in missing_strings:
